 ...
 *
 */
public class FlowInjectorHTTP extends PlayBase {

	public static final int FLOWINJECTION_APP_ID = 1129;
	public static final short IDLE_TO = 0;
	public static final short HARD_TO = 6000;

	public static final short inport = (short) 3;
	public static final MACAddress MAC_SRC = MACAddress
			.valueOf("8e:e2:89:b2:27:25");
	public static final MACAddress MAC_DST = MACAddress
			.valueOf("36:6d:6d:9f:6b:a4");
	...
	public static final byte NW_TYPE = (byte) 6;
	public static final byte PORT_SRC = 0;
	public static final short PORT_DST = 0;

	public static Logger log = LoggerFactory.getLogger(FlowInjectorHTTP.class);

	private Set<IDevice> devices;

	@Override
	public String getName() {
		return "Flow Injector";
	}
	...
	private void flowInjectProactive() {
		...
		match = new OFMatch();
		match.setDataLayerType(DL_TYPE).setNetworkSource(IP_MID)
				.setNetworkDestination(IP_DST).setNetworkProtocol(NW_TYPE);
		match.setWildcards(OFMatch.OFPFW_ALL & ~OFMatch.OFPFW_IN_PORT
				& ~OFMatch.OFPFW_DL_TYPE & ~OFMatch.OFPFW_NW_DST_MASK
				& ~OFMatch.OFPFW_NW_SRC_MASK & ~OFMatch.OFPFW_NW_PROTO);
		modifySrcAddress(s3, match, (short) 3, (short) 2, MAC_SRC.toBytes(),
				IP_SRC, cookie);
		...
		match.setWildcards(OFMatch.OFPFW_ALL & ~OFMatch.OFPFW_IN_PORT
				& ~OFMatch.OFPFW_DL_TYPE & ~OFMatch.OFPFW_NW_DST_MASK
				& ~OFMatch.OFPFW_NW_SRC_MASK & ~OFMatch.OFPFW_NW_PROTO);
		modifySrcAddress(s3, match, (short) 3, (short) 1, MAC_DST.toBytes(),
				IP_DST, cookie);
	}

	private void modifySrcAddress(IOFSwitch sw, OFMatch match, short inport,
			short outport, byte[] mod_mac, int mod_ip, long cookie) {
		match.setInputPort(inport);

		List<OFAction> actions = new ArrayList<OFAction>();
		OFActionDataLayer dl_action = new OFActionDataLayerSource(mod_mac);
		OFActionNetworkLayerAddress nw_action = new OFActionNetworkLayerSource(
				mod_ip);
	...
	private void installFlow(IOFSwitch sw, OFMatch match, short inport,
		...
		OFFlowMod fm = ((OFFlowMod) floodlightProvider.getOFMessageFactory()
				.getMessage(OFType.FLOW_MOD));
		fm.setIdleTimeout(FlowInjectorHTTP.IDLE_TO)
				.setHardTimeout(FlowInjectorHTTP.HARD_TO)
				.setCookie(cookie)
	...
	public void collectDevice(FloodlightContext cntx) {
		...
		devices.add(dstDevice);

		// log.info(
		// "Source Device {} attached to switch {} port {}",
		// new Object[] { srcDevice,
		// srcDevice.getAttachmentPoints()[0].getSwitchDPID(),
		// srcDevice.getAttachmentPoints()[0].getPort() });
		// log.info(
		// "Destination Device {} attached to switch {} port {}",
		// new Object[] { dstDevice,
